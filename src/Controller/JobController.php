...
class JobController
{
    ...
    public function getCompanyJobs($companyId, $quantity, $status)
    {

        // Check if quantity is larger than 0
        if ($quantity > 0) {
            // If quantity is greater than 0
            $jobQuery = $this->jobModel->selectCompanyJobs($companyId, $quantity, $status);
            if ($jobQuery) {
                // The job was successfully fetched.
                error_log('Job has been successfully fetched!');
                return $jobQuery;
    ...
    public function getCompanyJobById($companyId, $jobId)
    {
        try {
            $jobQuery = $this->jobModel->selectCompanyJobById($companyId, $jobId);
            if ($jobQuery) {
                // The fetch was successfully.
                error_log('Job has been successfully fetched!');
                return $jobQuery;
            } else {
                error_log('An error occurred during job fetch. Please try again.');
